	...
	<div class="card card-outline card-primary">
		<div class="card-header">
			<div class="card-tools">
				<a class="btn btn-block btn-sm btn-default btn-flat border-primary" href="./index.php?page=new_evaluator"><i class="fa fa-plus"></i> Agregar Nuevo Evaluador</a>
			</div>
		</div>
		<div class="card-body">
...
<script>
	...
	$('.view_evaluator').click(function(){
		uni_modal("<i class='fa fa-id-card'></i> Detalles de Evaluador","view_evaluator.php?id="+$(this).attr('data-id'))
	})
	$('.delete_evaluator').click(function(){
	_conf("¿Estás seguro de eliminar este evaluador?","delete_evaluator",[$(this).attr('data-id')])
	})
	})
	function delete_evaluator($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Datos eliminados correctamente",'proceso exitoso')
					setTimeout(function(){
						location.reload()
					},1500)
